...
module Identity
  class Plugin
    ...
    private def process_phone_number(text, params)
      ...
      OneLine::Store.data_for(params[:owner_id])["#{self.class}-data"][:mobile_phone_number] = phone_number
    ...
    private def send_verification_code(owner_id)
      ...
      phone_number = OneLine::Store.data_for(owner_id)["#{self.class}-data"][:mobile_phone_number]
      OneLine::Store.data_for(owner_id)["#{self.class}-data"][:verification_code] = verification_code
